...
module RubyCore
    ...
    class OreBlock < Block
      def initialize(options)
        ...
        super(AbstractBlock::Properties.create(@options[:material]).hardness_and_resistance(@options[:hardness],
                                                                                            @options[:resistance]).harvest_level(@options[:harvest_level]))

        # Create an item for the ore
        @block_item = BlockItem.new(self, Item::Properties.new.group(@options[:group]))

        @drop_item = Item.new(Item::Properties.new.group(@options[:item_drop_group])) unless @options[:drop_self]

        @block_form_block = Block.new(AbstractBlock::Properties.create(@options[:material]).hardness_and_resistance(
          @options[:hardness], @options[:resistance]
        ).harvest_level(@options[:harvest_level]))
        @block_form_block_item = BlockItem.new(@block_form_block, Item::Properties.new.group(@options[:group]))
      end

      def assign_options(options)
        @options[:material] = options[:material] || Material::STONE
        @options[:hardness] = options[:hardness] || 3
        @options[:resistance] = options[:resistance] || 10
        @options[:harvest_level] = options[:harvest_level] || 3
        @options[:group] = options[:group] || ItemGroup::TAB_BUILDING_BLOCKS
        @options[:name] = options[:name]
        @options[:drop_self] = options[:drop_self] || false
        @options[:block_form] = options[:block_form] || true
        @options[:item_drop_group] = options[:item_drop_group] || ItemGroup::TAB_MATERIALS
      end
      ...
      def get_experience_when_mined(_random)
        1
      end
    end
end
